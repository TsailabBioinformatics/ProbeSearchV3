...
//app.put('/',  async (req, res) => {
function rAligner(input,db,X,maxHit,gapFlag) {
	...
	return new Promise((resolve, reject) => { 
		...
		if (input.length > 50) {
			exec('./bowtie/bowtie2 -x indices/' + String(req.body.db) + '/' + String(req.body.db) + ' -k 30 -c ' + String(req.body.read) + '  --very-sensitive --no-hd' , 
				...
				// make fasta input
				execSync('echo ">seq\n' + input + '" > ' + ts + "input.fa"); 
				exec('razers3 -i '+ errorPercentage +' -m '+ maxHit +' -tc 4 --a '+ gap +' -o '+ ts + 'output.razers /home/webserver/probesearch/data/'+String(db)+'/'+'*.fa ' + ts +'input.fa', 
					(error, stdout, stderr) => {
...
module.exports = {rAligner}; // Export the function
